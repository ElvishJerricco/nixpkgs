...
let
  ...
  makeFstabEntries =
    ...
    inInitrdFstab = fs: utils.fsNeededForBoot fs && !(builtins.elem fs.mountPoint ["/" "/usr"]);
    initrdFstab = pkgs.writeText "initrd-fstab" (makeFstabEntries (filter inInitrdFstab fileSystems) {
      rootPrefix = "/sysroot";
    });
