...
let
  ...
  fileSystemOpts = { config, ... }: {
    ...
    config.options = mkMerge [
      (mkIf config.autoResize [ "x-nixos.autoresize" "x-systemd.growfs" ])
...

{
  ...
  config = {
    ...
    assertions = let
      ...
      resizableFSes = if config.boot.initrd.systemd.enable then [
        "ext3"
        "ext4"
        "btrfs"
        "xfs"
    ...
    in [
      ...
      { assertion = ! (any notAutoResizable fileSystems);
        ...
        in ''
          ...
          ${if fs.fsType == "auto" then "fsType has to be explicitly set and" else ""}
